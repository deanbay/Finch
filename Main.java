...
public class Main {
	...
	static ArrayList<Sqr> listofsquares = new ArrayList<Sqr>();//storing an array of squares where the getters are stored
	static ArrayList<Tri> listoftriangles = new ArrayList<Tri>(); //storing an array of triangles where the getters are stored
	...
	/**
	 * Uses JOptionPane so user can choose triangle, rectangle or quit.
	 * 
	 * 
	...
	/**
	 * Opens the GUI so the user can enter the dimension for the rectangle.
	 * Code here to take input and convert into Square object.
	 * Checks whether values are valid or not.
	 * 
	 */
	public static void SquareGUI() {
		...
		int result = JOptionPane.showConfirmDialog(null, myPanel, "Draw square", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE, null);
		...
		if (result == JOptionPane.OK_OPTION) {
			...
			}catch(Exception e) {JOptionPane.showMessageDialog(null, "No text allowed, please enter numerical values! ");}	//eliminating runtime errors by catching any exception		
	...
	public static void drawSqr(Sqr r1) {
		...
		myFinch.setWheelVelocities(v, v, (len + 5) * lenMultiplier); //getting  the length which is then multiplied by 5, and then by 100 as we are dealing with milliseconds based on the finch
	...
	public static void TriangleGUI() {
		...
		int result = JOptionPane.showConfirmDialog(null, myPanel, "Draw Triangle", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE, null); //popup message with getters and and setters in place
	...
	public static boolean CanTriangleBeDrawn(Tri t1) {
		...
		if(len1 > len2 + len3 || (len2 > len1 + len3) || (len3 > len2 + len1)){
			JOptionPane.showMessageDialog(null, "The values you've entered equate to the triangle inequality therom, one of your side values is greater than or eqaul to that of the sum of opposing sides and therefore cant be drawn."); return false;
		}
		if(len1 + len2 == len3 || (len2 + len1 == len3) || (len3 + len2 == len1)){
			JOptionPane.showMessageDialog(null, "The values you've entered equate to the triangle inequality therom, one of your side values is greater than or eqaul to that of the sum of the opposing sides and therefore cant be drawn."); return false;
		}
		return true;
	}
	...
	public static void outputResults() {
		...
		int result = JOptionPane.showConfirmDialog(null, "Total shapes drawn: "+ myPanel +"\n"+ "Thank you for testing my program!", "Results", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);


		System.out.println("== Total Number of Shapes drawn: " + total +" ==");

		printTriangles (listoftriangles); 
		printSquares(listofsquares);
		System.out.println("This is the average time taken drawing: "+TimeCollector.getAverage()); // this is the average time taken
	...
	private static void printTriangles(ArrayList<Tri> listoftriangles2) {
		// TODO Auto-generated method stub
		for(int i=0; i<listoftriangles2.size();i++) { //goes through the list one by one until it gets to one less than the size array list 
			System.out.println("=================");
			System.out.println("This is Triangle number: " + (i+1));// otherwise the list begins with 0 
			System.out.println("This is the lengths: " + listoftriangles2.get(i).getL1() +", "+ listoftriangles2.get(i).getL2()+", "+ listoftriangles2.get(i).getL3());
			System.out.println("This is the angles: " + listoftriangles2.get(i).getA() +", "+ listoftriangles2.get(i).getB()+", "+ listoftriangles2.get(i).getC());
	...
	private static void printSquares(ArrayList<Sqr> listofsquares2) {
		// TODO Auto-generated method stub
		for(int i=0; i<listofsquares2.size();i++) { //goes through the list one by one until it gets to one less than the size array list 
			System.out.println("=================");
